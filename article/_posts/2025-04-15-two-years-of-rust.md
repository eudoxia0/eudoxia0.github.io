...
When I worked with Python+Django the characteristic feeling was _anxiety_. Writing Python feels like building a castle out of twigs, and the higher you go, the stronger the wind gets. You expect things to go wrong, you expect the code to be slow, you expect that when you run a script you will wait for a few hundred milliseconds before anything happens. In JavaScript the characteristic feeling is _disappointment_: at least with Python the twigs are rigid, whereas writing JavaScript feels like trying to shape mud into a house, you constantly have to go back and forth re-shaping the parts that are melting in the sun, and when you suggest using non-liquid building material you're told to get off the ivory tower, because nobody gives a shit about the quality of the construction.
